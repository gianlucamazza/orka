...
pub fn inspect_config_issues(raw: &str) -> Result<Vec<String>, MigrationError> {
    ...
    inspect_coding_warnings(&doc, &mut warnings);
...
/// v6 → v7: promote `[os.coding]` to a top-level `[coding]` section.
///
/// Coding delegation (Claude Code, Codex, `OpenCode`) is independent of OS
/// integration. The config section moves from `[os.coding]` to `[coding]`.
fn migrate_v6_to_v7(doc: &mut DocumentMut, warnings: &mut Vec<String>) {
    // Extract and remove [os.coding] if it exists.
    let coding_item = doc
        .get_mut("os")
        .and_then(|os| os.as_table_mut())
        .and_then(|os| os.remove("coding"));

    if let Some(item) = coding_item {
        if doc.get("coding").is_none() {
            doc.insert("coding", item);
        }
        warnings.push(
            "[os.coding] has been promoted to top-level [coding]. \
             Run `orka config migrate` to persist this change."
                .into(),
        );
    }

    doc.insert("config_version", value(7i64));
}
...
fn inspect_os_warnings(doc: &DocumentMut, warnings: &mut Vec<String>) {
    ...
    // Warn if [os.coding] is still present (should have been promoted by v6→v7
    // migration).
    if doc.get("os").and_then(|os| os.get("coding")).is_some() {
        warnings.push(
            "config key [os.coding] is legacy as of v7; it should have been promoted to \
             top-level [coding] by the migration. Run `orka config migrate` to fix this."
                .into(),
        );
    }
}
...
fn inspect_coding_warnings(doc: &DocumentMut, warnings: &mut Vec<String>) {
    let coding_allowed = [
        "enabled",
        "default_provider",
        "selection_policy",
    ...
    inspect_table_keys(doc, &["coding"], &coding_allowed, &[], &[], warnings);
    inspect_table_keys(
        doc,
        &["coding", "providers"],
...
"#;
    ...
    fn future_version_rejected() {
        ...
        assert!(matches!(
            err,
            MigrationError::FutureVersion { found: 999, max: 7 }
        ));
