...
///
/// Mirrors the internal `Payload` enum but belongs to the contracts crate so
/// adapters can depend on it without pulling in all of `orka-core`. The bridge
/// layer converts between these two types at the single integration boundary.
///
