//! Coding delegation configuration types for Orka.
    ...
    pub fn validate(&self) -> Result<()> {
            ...
            && !self.providers.opencode.enabled
        {
            return Err(Error::Config(
                "coding.enabled requires at least one enabled provider under coding.providers"
                    .into(),
            ));
        }
...
impl ClaudeCodeConfig {
    ...
    pub fn validate(&self) -> Result<()> {
        if self.timeout_secs == 0 {
            return Err(Error::Config(
                "coding.providers.claude_code.timeout_secs must be greater than zero".into(),
            ));
        }
        Ok(())
    }
...
impl CodexConfig {
    ...
    pub fn validate(&self) -> Result<()> {
        if self.timeout_secs == 0 {
            return Err(Error::Config(
                "coding.providers.codex.timeout_secs must be greater than zero".into(),
            ));
        }
        Ok(())
    }
...
impl OpenCodeConfig {
    ...
    pub fn validate(&self) -> Result<()> {
        if self.timeout_secs == 0 {
            return Err(Error::Config(
                "coding.providers.opencode.timeout_secs must be greater than zero".into(),
            ));
        }
        Ok(())
    }
