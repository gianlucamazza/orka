...
use crate::{
    config::{
        ...
        OpenCodeConfig, SandboxMode,
    },
    stream::DelegateEvent,
};
    ...
    fn parse(input: &SkillInput, config: &CodingConfig) -> Result<Self> {
        ...
        if config.require_verification && verification.is_none() {
            return Err(Error::SkillCategorized {
                message: "missing 'verification' argument: coding.require_verification = true"
                    .into(),
                category: ErrorCategory::Input,
            });
        }
...
fn schema() -> SkillSchema {
    SkillSchema::new(serde_json::json!({
        ...
        "properties": {
            ...
            "working_dir": {
                "type": "string",
                "description": "Optional working directory override. Only honored when coding.allow_working_dir_override = true."
            }
        },
        "required": ["task"]
    }))
...
impl CodingDelegateSkill {
    /// Create a routing skill from the coding configuration.
    pub fn new(config: &CodingConfig) -> Self {
        Self {
            coding: config.clone(),
            claude: ClaudeCodeBackend::new(&config.providers.claude_code),
            codex: CodexBackend::new(&config.providers.codex),
            opencode: OpenCodeBackend::new(&config.providers.opencode),
        }
    }
    ...
    fn select_backend(&self) -> Result<&dyn CodeDelegateBackend> {
        ...
        chosen.ok_or_else(|| Error::SkillCategorized {
            message: "no coding backend available: enable a provider under coding.providers".into(),
...
impl CodeDelegateBackend for ClaudeCodeBackend {
    ...
    async fn run(
        ...
        execute_command_streaming(
            self,
            cmd,
            crate::stream::parse_claude_stream_line,
            parse_claude_output,
...
impl CodeDelegateBackend for CodexBackend {
    ...
    async fn run(
        ...
        execute_command_streaming(
            self,
            cmd,
            crate::stream::parse_codex_stream_line,
            parse_codex_output,
            input.context.as_ref().and_then(|c| c.progress_tx.clone()),
            input
                .context
...
impl CodeDelegateBackend for OpenCodeBackend {
    ...
    async fn run(
        ...
        execute_command_streaming(
            self,
            cmd,
            crate::stream::parse_opencode_stream_line,
...
mod tests {
    ...
    fn base_coding_config() -> CodingConfig {
        let mut config = CodingConfig::default();
        config.enabled = true;
        config.default_provider = CodingProvider::Auto;
    ...
    fn schema_requires_task() {
        let skill = CodingDelegateSkill::new(&base_coding_config());
    ...
    fn prompt_contains_no_workaround_constraint() {
        let config = base_coding_config();
        let backend = ClaudeCodeBackend::new(&config.providers.claude_code);
    ...
    fn auto_selection_prefers_configured_policy() {
        let mut config = base_coding_config();
        config.selection_policy = CodingSelectionPolicy::PreferCodex;
    ...
    fn direct_backend_uses_explicit_default() {
        let mut config = base_coding_config();
        config.default_provider = CodingProvider::ClaudeCode;
    ...
    fn opencode_direct_backend_selection() {
        let mut config = base_coding_config();
        config.default_provider = CodingProvider::OpenCode;
    ...
    fn prefer_opencode_policy_selects_opencode_first() {
        let mut config = base_coding_config();
        config.selection_policy = CodingSelectionPolicy::PreferOpenCode;
        config.providers.opencode.enabled = true;
        let skill = CodingDelegateSkill::new(&config);
        let selected = skill.select_backend().unwrap();
        assert_eq!(selected.kind(), BackendKind::OpenCode);
