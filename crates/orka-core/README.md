...
| `config`  | Shared config primitives (`defaults`, `primitives`) and agent/graph config (`agent`); domain config lives in owning crates, composed `OrkaConfig` in `orka-config` |
